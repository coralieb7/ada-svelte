<script>
	...
	onMount(() => {
		function wordCloud(selector) {
			...
			function draw(words) {
				...
				text
					...
					.attr('transform', (d) => `translate(${d.x}, ${d.y/2})rotate(${d.rotate})`);
		...
		// Fixed word list
		const words = [
			{ text: 'FinalGirl', size: 20, color: '#17d07d' },
			{ text: 'BrokenBird', size: 19, color: '#17d07d' },
			{ text: 'DumbBlonde', size: 15, color: '#17d07d' },
			{ text: 'BrainlessBeauty', size:13, color: '#17d07d' },
			{ text: 'ValleyGirl', size: 11, color: '#17d07d' },
			{ text: 'ClassyCatBurglar', size: 9, color: '#17d07d' }
		];
